...
class WordCountAndTimePlugin 
{
    ...
    function createHTML($content) {
        ...
        // ejecutar word count uan vez porque wordcount y read time lo necesitarán
        ...
        if (get_option('wcp_readtime', '1')) {
            $html .= esc_html__('This post will take', 'wcpdomain') . ' ' . round($wordCount/225) . ' ' . esc_html__('minute(s) to read', 'wcpdomain') . '.<br>';
        }
    ...
    function sanitizeLocation($input) {
        if ($input != '0' AND $input != '1') {
            add_settings_error('wcp_location', 'wcp_location_error', 'Display location debe ser el inicio o final');
...
<select name="wcp_location">
    <option value="0" <?php selected(get_option('wcp_location'), '0'); ?>>Inicio del post</option>
    <option value="1" <?php selected(get_option('wcp_location'), '1'); ?>>Final del post</option>
</select>
<?php }
